...
from nets.facenet.facenet import Facenet
...
class FaceRecognition(object):
    _defaults = {
        ...
        #   数据库编码根目录
        #--------------------------------#
        "database_code_dir"     : "data/database_code",
        #--------------------------------#
        #   是否使用Cuda
        #   没有GPU可以设置成False
        #--------------------------------#
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        if encoding:
            ...
            try:
                ## 从文件加载的两个数组，分别是(n,128)以及(n,)
                self.known_face_encodings = np.load("{code_root}/{backbone}_face_encoding.npy".format(code_root=self.database_code_dir,backbone=self.facenet_backbone))
                self.known_face_names     = np.load("{code_root}/{backbone}_names.npy".format(code_root=self.database_code_dir,backbone=self.facenet_backbone))

                print(self.known_face_encodings.shape)
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        np.save("data/database_code/{backbone}_names.npy".format(backbone=self.facenet_backbone),names)
        np.save("data/database_code/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone),face_encodings)
